              ...
              <div className="flex-1 pb-12">
                <div className="bg-linear-to-br from-white/10 to-white/5 dark:from-white/10 dark:to-white/5 light:from-white light:to-slate-50 border border-white/10 dark:border-white/10 light:border-slate-200 rounded-2xl p-6 hover:border-white/20 dark:hover:border-white/20 light:hover:border-slate-300 transition-all duration-300">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-2xl font-bold text-white dark:text-white light:text-slate-900 mb-1">{exp.role}</h3>
                      <p className="text-cyan-400 dark:text-cyan-400 light:text-cyan-600 font-medium">{exp.company}</p>
                    </div>
                    <span className="px-4 py-1.5 bg-purple-500/20 dark:bg-purple-500/20 light:bg-purple-100 border border-purple-500/30 dark:border-purple-500/30 light:border-purple-300 rounded-full text-sm text-purple-300 dark:text-purple-300 light:text-purple-700 font-medium">
                      {exp.year}
                    </span>
                  </div>

                  <p className="text-white/60 dark:text-white/60 light:text-slate-600 mb-4">{exp.description}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.achievements.map((achievement) => (
                      ...
                      >
                        <Star className="w-3 h-3 text-yellow-400 dark:text-yellow-400 light:text-yellow-600" />
                        {achievement}
                      </span>
                    ))}
                  </div>
